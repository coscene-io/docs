...
[//]: # ()
[//]: # ()
[//]: # (可以使用 `get_value&#40;key&#41;` 函数获取模版化中的参数，例如上图中模版化中的参数组中有一个 Key 为 `errMsg` 的参数，)

[//]: # ()
[//]: # (那么在触发条件中即可使用 `get_value&#40;'errMsg'&#41;` 来获取该参数的值。)

[//]: # ()
[//]: # ()
[//]: # (一个示例触发条件是 `get_value&#40;'errMsg'&#41; in log`, 如果用带有这个条件的规则模版和上图中的参数组创建规则，)

[//]: # ()
[//]: # (那么用第一个参数组创建的规则会有一个条件等价于 `"error 1" in log`，用第二个参数组创建的规则会有一个条件等价于 `"error 2" in log`。)
